...
#define DebugCout(x) setConsoleColor(ADDITIONAL); std::cout << x << std::endl; setConsoleColor(DEFAULTCOL);
...
namespace phm
{
		...
		: window_{ window }
	{
		...
		DebugCout("Starting Instance Creation");
		createInstance();
		DebugCout("Setting up debug messenger");
		setupDebugMessenger();
		DebugCout("Creating surface");
		createSurface();
		DebugCout("Picking physical device");
		pickPhysicalDevice();
		DebugCout("Creating logical device");
		createLogicalDevice();
		DebugCout("Creating command pool");
		createCommandPool();
	}
	...
	bool Device::checkValidationLayerSupport()
	{
		...
		for (const char* layerName : validationLayers)
		{
			...
			if (!layerFound)
			{
				DebugCout("Layer was not found during Validation Layer Support checking! Layer name is: " << layerName);
				return false;
			}
		}
